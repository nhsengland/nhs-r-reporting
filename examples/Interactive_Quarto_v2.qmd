...
Click on the visual tab and there are all the usual formatting options.
...
So if I put in <backtick>r format(Sys.Date(), '%d %B %Y') <backtick>, this would return todays date.  Look at the code to see it in action.
...
You can also pull data from data frames such as the highest amount of attendances in our data set was `r format(max(data$attendances), big.mark= ',')`.  The format function is really nice at making human readable numbers or you can get R to convert numbers to text to say the highest was `r words(max(data$attendances))` attendances.  There are also options to add appropriate indefinite articles before numeric words and convert numbers into ordinals such as 'first', 'third' etc.  
...
We can now render these in separate tabs.
